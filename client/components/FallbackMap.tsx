...
export const FallbackMap: React.FC<FallbackMapProps> = ({
  ...
  const getMapCenter = (): [number, number] => {
    if (pickup && destination) {
      return [(pickup.lat + destination.lat) / 2, (pickup.lng + destination.lng) / 2];
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-sm text-gray-600 bg-blue-50 p-3 rounded-lg">
        <p>
          <strong>How to use:</strong> Click on the map to set your {locationMode === "pickup" ? "pickup" : "destination"} location,
          or search for a place using the search bar above.
        </p>
      </div>
