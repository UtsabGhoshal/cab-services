...
class DriverService {
  ...
  async getNearbyRides(driverLocation: DriverLocation, radiusKm: number = 10): Promise<RideRequest[]> {
    try {
  ...
  // Update ride status
  async updateRideStatus(rideId: string, status: RideRequest['status']): Promise<boolean> {
    try {
      const { error } = await supabase
        .from('rides')
        .update({
  ...
  async getDriverStats(driverId: string): Promise<DriverStats> {
    try {
      ...
      const totalEarnings = completedRides?.reduce((sum, ride) => sum + (ride.fare || 0), 0) || 0;
  ...
  subscribeToDriverRides(driverId: string, callback: (rides: RideRequest[]) => void) {
