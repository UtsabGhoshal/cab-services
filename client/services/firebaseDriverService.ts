...
export class FirebaseDriverService {
  ...
  // Vehicle operations
  async createVehicle(
    vehicleData: Omit<FirebaseVehicle, "id" | "createdAt" | "updatedAt">,
  ): Promise<string> {
    const now = Timestamp.now();
  ...
  async updateVehicle(
    vehicleId: string,
    updates: Partial<FirebaseVehicle>,
  ): Promise<void> {
  ...
  // Ride operations
  async createRide(
    rideData: Omit<FirebaseRide, "id" | "createdAt" | "updatedAt">,
  ): Promise<string> {
    const now = Timestamp.now();
  ...
  async updateCommissionSettings(
    settings: Omit<CommissionSettings, "id" | "updatedAt">,
    updatedBy: string,
  ): Promise<void> {
  ...
  onVehiclesChange(
    callback: (vehicles: FirebaseVehicle[]) => void,
  ): () => void {
    const q = query(this.vehiclesCollection, orderBy("createdAt", "desc"));
    return onSnapshot(q, (snapshot) => {
  ...
  onDriverRidesChange(
    driverId: string,
    callback: (rides: FirebaseRide[]) => void,
  ): () => void {
    const q = query(
      this.ridesCollection,
      where("driverId", "==", driverId),
